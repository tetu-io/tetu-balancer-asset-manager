...
contract TetuStablePool is BaseGeneralPool, BaseMinimalSwapInfoPool, StableMath, IRateProvider {
  ...
  function _onInitializePool(
    ...
    _require(kind == TetuStablePool.JoinKind.INIT, Errors.UNINITIALIZED);
  ...
  function _isOwnerOnlyAction(bytes32 actionId) internal view virtual override returns (bool) {
    return
    (actionId == getActionId(TetuStablePool.startAmplificationParameterUpdate.selector)) ||
    (actionId == getActionId(TetuStablePool.stopAmplificationParameterUpdate.selector)) ||
